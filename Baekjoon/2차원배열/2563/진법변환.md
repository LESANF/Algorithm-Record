...
```js
가로, 세로 100이라는 정사각형의 넓이를 정해줬다.

색종이들의 넓이를 구하기위해 겹치는 부분의 넓이가 필요하다.

좌표를 부여해주기 때문에 100*100 이차원 배열을 false로 초기화 하여 생성한다.

색종이 좌표들 전부 true 로 바꿔준다. 겹치는 부분도 1이기때문에 중복을 제거할 수 있다.
